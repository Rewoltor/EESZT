...
function findExactMatches(
    ...
    const searchTerms = [testName, ...(aliases || [])].map((t) =>
        t.toLowerCase().trim()
    );
 ...
 */
function extractValueNearMatch(
    items: TextItem[],
    match: MatchPosition
): number | null {
...
function extractDateFromBottom(items: TextItem[]): string | null {
    ...
    const bottomItems = items.filter(
        (item) => item.transform[5] < bottomThreshold
    );
    ...
    // Pattern 2: Leletnyomtatás/mentés: YYYY.MM.DD
    const pattern2 =
        /Leletnyomtatás\s*\/\s*mentés:\s*(\d{4})\.(\d{2})\.(\d{2})/i;
...
function buildResult(
    ...
    const result: BloodTestResult = {
        ...
        ref_min: testDef.ref_min,
        ref_max: testDef.ref_max,
        category: testDef.category,
        aliases: testDef.aliases,
    };
